...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        resp = generate_legacy("fast", prompt, api_key=_get_api_key())
...
def computer_settings(
    ...
    if action in ("wake_status", "wake_check", "jarvis_wake_status"):
        base = _get_base_dir()
        launch = Path.home() / "Library" / "LaunchAgents" / "com.jarvis.wake.plist"
        app = Path("/Applications/JARVIS.app")
        log = Path.home() / "Library" / "Logs" / "mark_xxxix_wake.log"
        state = []
        state.append(f"launch_agent={'ok' if launch.exists() else 'missing'}")
        state.append(f"app={'ok' if app.exists() else 'missing'}")
        state.append(f"listener_log={'ok' if log.exists() else 'missing'}")
        try:
            out = subprocess.run(["pgrep", "-fl", "wake_listener.py"], capture_output=True, text=True, timeout=2)
            state.append(f"process={'running' if 'wake_listener.py' in out.stdout else 'stopped'}")
        except Exception:
            state.append("process=unknown")
        return "Wake diagnostics: " + ", ".join(state)
