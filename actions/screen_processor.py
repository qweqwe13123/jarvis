...
class _CameraCache:
    ...
    def capture(self, max_age: float = 0.25) -> tuple[bytes, str]:
        if not _CV2:
            raise RuntimeError("OpenCV (cv2) is not installed. Run: pip install opencv-python")

        now = time.monotonic()
        with self._lock:
            if self._last_frame and now - self._last_ts <= max_age:
                return self._last_frame, self._last_mime
...
_camera_cache = _CameraCache()
atexit.register(_camera_cache.close)
...
def _throttle_vision_request() -> None:
    ...
    now = time.monotonic()
    elapsed = now - _last_vision_ts
    if elapsed < _VISION_MIN_INTERVAL:
        time.sleep(_VISION_MIN_INTERVAL - elapsed)
    _last_vision_ts = time.monotonic()
...
def screen_process(
    ...
    if not user_text:
        ...
        return "No question provided for vision."
