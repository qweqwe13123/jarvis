...
def _launch_macos(app_name: str) -> bool:
    ...
    simplified = target.lower().replace(".app", "").strip()
    candidates = [p for p in app_root.glob("*.app") if simplified in p.stem.lower()]
    if candidates:
        try:
            result = subprocess.run(["open", str(candidates[0])], capture_output=True, timeout=8)
            if result.returncode == 0:
                time.sleep(1.0)
                return True
        except Exception:
    ...
    script_name = target.replace(".app", "")
    try:
        result = subprocess.run(
            ["osascript", "-e", f'tell application "{script_name}" to activate'],
            capture_output=True,
            timeout=8,
        )
        if result.returncode == 0:
