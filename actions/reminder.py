...
from core.language import load_language, phrase
...
def _spoken_message(message: str) -> str:
    clean = _sanitise(message, 160) or "ваше напоминание"
    lower = clean.lower()
    lang = load_language()
    if "вода" in lower or "вод" in lower or "water" in lower or "su" == lower:
        variants_by_lang = {
            "ru": [
                "Босс, ваше время выпить воду наступило.",
                "Сэр, напоминаю: пора выпить воды.",
                "Босс, небольшой перерыв. Время выпить воду.",
            ],
            "tr": [
                "Patron, su içme zamanı geldi.",
                "Efendim, hatırlatıyorum: su içme zamanı.",
            ],
            "az": [
                "Boss, su içmək vaxtı gəldi.",
                "Cənab, xatırladıram: su içmək vaxtıdır.",
            ],
            "en": [
                "Boss, it is time to drink water.",
                "Sir, quick reminder: time to drink water.",
            ],
        }
        variants = variants_by_lang.get(lang, variants_by_lang["ru"])
    else:
        variants_by_lang = {
            "ru": [f"Босс, напоминаю: {clean}.", f"Сэр, наступило время: {clean}.", f"Босс, ваше напоминание: {clean}."],
            "tr": [f"Patron, hatırlatma: {clean}.", f"Efendim, zamanı geldi: {clean}."],
            "az": [f"Boss, xatırlatma: {clean}.", f"Cənab, vaxtı gəldi: {clean}."],
            "en": [f"Boss, reminder: {clean}.", f"Sir, it is time: {clean}."],
        }
        variants = variants_by_lang.get(lang, variants_by_lang["ru"])
...
def _write_notify_script(task_name: str, message: str, os_name: str) -> Path:
    ...
    spoken_literal = json.dumps(_spoken_message(message), ensure_ascii=False)

    if os_name == "windows":
        notify_block = f"""
message = {msg_literal}
spoken = {spoken_literal}
notified = False
...
try:
    ...
    if speaker:
        subprocess.run([speaker, spoken], check=False)
except Exception:
    pass
"""
...
def _schedule_short_timer(delay_seconds: int, script_path: Path) -> str:
    ...
    return f"timer_{int(time_module.time())}"
...
def reminder(
    ...
    try:
        if delay_seconds is not None and delay_seconds <= 3600:
            job_id = _schedule_short_timer(delay_seconds, script_path)
    ...
    if player:
        player.write_log(f"[Reminder] ✅ {target_dt.strftime('%Y-%m-%d %H:%M:%S')} — {safe_msg[:40]}")
