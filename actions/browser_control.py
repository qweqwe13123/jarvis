...
def _looks_like_url(text: str) -> bool:
    ...
    if " " in text:
        return False
    return bool(re.search(r"^[a-z0-9][a-z0-9\-]*(\.[a-z0-9][a-z0-9\-]*)+(:\d+)?(/.*)?$", text))


def _open_system_browser(url: str, browser: str | None = None) -> str:
    ...
    try:
        if _OS == "Darwin":
            ...
            app = app_map.get((browser or "").lower().strip())
        ...
        return f"System browser open failed for {url}: {e}"
    return f"System browser open is not available for {url}"
...
def browser_control(
    ...
    if action == "go_to":
        ...
        if url:
            system_first = params.get("system_first", True)
            if system_first:
                result = _open_system_browser(url, browser)
                _log(player, result)
                return result
    elif action == "search" and _looks_like_url(params.get("query", "")):
        result = _open_system_browser(params.get("query", ""), browser)
        _log(player, result)
        return result
    elif not action and (params.get("url") or _looks_like_url(params.get("query", ""))):
        result = _open_system_browser(params.get("url") or params.get("query", ""), browser)
        _log(player, result)
        return result
    ...
    try:
        ...
        if action == "go_to" and params.get("url"):
            result = _open_system_browser(params.get("url", ""), browser)
        else:
