...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "'on Android' → platform=android. Phone supports: open_url = search the web or open a "
            "link (pass url, or query). Pick engine from the user's words: engine=google (default, "
            "also 'хром/chrome'), engine=yandex ('в яндексе'), engine=youtube ('на ютубе/видео'), "
            "engine=bing, engine=duckduckgo. Works for movies, series, news, facts, any site; for "
            "films AURA auto-adds 'смотреть онлайн'. Also: action=open_last (continue last opened), "
            "action=lock (lock screen), action=play|pause. E.g. 'найди на телефоне сериал X' → "
            "platform=android, kind=open_url, query=X. 'найди в яндексе новости про Y' → "
            "platform=android, engine=yandex, query=новости про Y. The phone CANNOT fully shut "
            "down. No app/file control on the phone. "
