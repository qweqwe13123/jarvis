...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "'on Android' → platform=android. Phone supports: open_url (url, or query for a "
            "movie/search; engine=youtube default or engine=google for 'в гугле'); action=open_last "
            "to continue the last opened thing ('продолжи фильм', 'открой последнее'); action=lock "
            "to lock the screen ('заблокируй телефон'); action=play|pause. E.g. 'найди на телефоне "
            "фильм Inception' → platform=android, kind=open_url, query=Inception. The phone CANNOT "
            "fully shut down. No app/file control on the phone. "
